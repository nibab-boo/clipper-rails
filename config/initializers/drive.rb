...
class MyDrive
  OOB_URI = "#{ENV['BASE_URL']}/oauth2callback".freeze
  ...
  def from_code(code, user_id)
    @credentials = @authorizer.get_credentials_from_code(
      user_id: "#{user_id}", code: code, scope: @scope[0], base_url: OOB_URI)
  end

  def save_credentials(user_id)
    @authorizer.store_credentials(user_id.to_s, @credentials)
    @drive.authorization = @credentials
  end
  ...
  def get(id, options = {})
    p Dir.pwd
    @drive.get_file(id, download_dest: 'lib/assets/downloaded-test.txt')
  end
